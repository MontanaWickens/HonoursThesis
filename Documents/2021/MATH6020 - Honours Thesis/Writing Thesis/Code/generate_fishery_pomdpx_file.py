...
import world_model_functions as w
...
if __name__ == "__main__":
    ...
    parser.add_argument('-R', type=int, action="store", dest='R', default=3)
    ...
    try:
        biomass_vector = [(int(i*B_max/N_B),int((i+1)*B_max/N_B)) for i in range(N_B)]
    except:
    ...
    transition = f.discrete_BH_transition(states_vector, action_vector)
    
    # observation transition matrix
    obs_probs = f.observation_function(states_vector, action_vector, observations_vector)
    ...
    ## variables
    # state: stock size and harevst rate
    ...
    # action
    text.append('<ActionVar vname="stock_assessment_decision">'
    ...
    text.append('<StateTransitionFunction>'
          '<CondProb>' 
          '<Var>b-h_1</Var>'
          '<Parent>stock_assessment_decision b-h_0</Parent>'
          '<Parameter type="TBL">')
    ...
    text.append('<ObsFunction>'
      '<CondProb>' 
      '<Var>catch</Var>'
      '<Parent>stock_assessment_decision b-h_1</Parent>'
      '<Parameter type="TBL">')
    ...
    import os
    filename = os.path.join(os.getcwd(), "a%d-s%d-r%d-k%d_modelA.pomdpx" % (args.a, args.s, args.r, args.k))
...
np.save('transition_fishery_model.npy', transition)
np.save('obs_prob_fishery_model', obs_probs)
np.save('action_vector_fishery_model.npy', action_vector)
np.save('biomass_vector_fishery_model.npy', biomass_vector)
np.save('K_vector_fishery_model.npy', K_vector)
np.save('r_vector_fishery_model.npy', r_vector)